...
class MessageSerializer(serializers.ModelSerializer):
    ...
    Thread detail uses ``MessageThreadDetailSerializer.get_messages`` with
    ``context=self.context`` so nested rows receive ``request`` for absolute
    media/download URLs.
    """
    ...
    def get_sender_role(self, obj):
        ...
        if (
            getattr(obj.sender, 'is_staff', False)
            or getattr(obj.sender, 'is_superuser', False)
        ):
            return 'admin'
    ...
    def get_attachment_url(self, obj):
        """Return an absolute download URL for an attachment."""
...
class MessageThreadSerializer(serializers.ModelSerializer):
    ...
    def get_can_edit_background(self, obj):
        ...
        return user_can_edit_thread_background(
            request.user,
            obj,
        )
    ...
    def get_last_message_preview(self, obj):
        """
        Snippet for inbox lists:
        prefer filename for attachments;
        map legacy ``(attachment)`` text.
        """
...
class MessageThreadBackgroundSerializer(serializers.ModelSerializer):
    ...
    clear_background_image = serializers.BooleanField(
        required=False,
        write_only=True,
        default=False,
    )

    clear_wallpaper_image = serializers.BooleanField(
        required=False,
        write_only=True,
        default=False,
    )
    ...
    def _validate_image(self, value):
        ...
        if not content_type.startswith('image/'):
            raise serializers.ValidationError(
                'Upload an image file (PNG, JPG, WebP, etc.).'
            )
    ...
    def update(self, instance, validated_data):
        ...
        clear_cover = validated_data.pop(
            'clear_background_image',
            False,
        )
        ...
        clear_wallpaper = validated_data.pop(
            'clear_wallpaper_image',
            False,
        )
...
class MessageThreadDetailSerializer(MessageThreadSerializer):
    """
    Thread including ordered ``messages`` (timeline).
    Nested serializer receives parent ``context``.
    """
